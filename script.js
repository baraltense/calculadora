...
customRateInput.addEventListener('input', function() {
    validateDecimalPlaces(this, 6);
    if (this.value.length > 10 && !isNaN(parseFloat(this.value))) {
...
function convertCurrency() {
    ...
    if (isNaN(amount) && amountInput.value.trim() !== '') {
        ...
        conversionResult.textContent = ''; // Resultado vacío
...
function initializeCalculator() {
    ...
    amountInput.value = '';
    customRateInput.value = '';
    conversionResult.textContent = ''; // Resultado inicial vacío
    conversionResult.classList.add('conversion-result-styled');
    copyBtn.style.display = 'none';
    amountError.textContent = '';
    customRateError.textContent = '';
